...
export const FeedbackModal: React.FC<FeedbackModalProps> = ({ onClose, showNotification }) => {
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden max-w-md w-full border border-slate-100 shadow-2xl font-sans flex flex-col">
      ...
      <div className="p-6 overflow-y-auto flex-1">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Kirim Sebagai</label>
            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200">
              ...
              />
              <label htmlFor="anonimCheck" className="text-sm font-semibold text-slate-700 cursor-pointer select-none">
          ...
          {!isAnonymous && (
            <div className="space-y-1.5 animate-fade-in">
              <label className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Nama Anda</label>
              <div className="flex items-center bg-white border border-slate-300 focus-within:border-teal-500 rounded-xl overflow-hidden px-3.5 py-2.5">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-600 uppercase tracking-wider block">Pesan Saran &amp; Masukan</label>
            <textarea
              placeholder="Tambahkan fitur ini itu..."
              value={feedback}
              onChange={(e) => setFeedback(e.target.value)}
              rows={4}
              className="w-full bg-white border border-slate-300 focus:border-teal-500 rounded-xl px-3.5 py-3 text-sm font-medium focus:outline-none resize-none placeholder:text-slate-300 text-slate-800"
              disabled={isSubmitting}
            />
          </div>

          {!GOOGLE_DOCS_FEEDBACK_WEBAPP_URL && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2.5 text-amber-800 text-[11px] leading-relaxed">
          ...
          <div className="pt-2 flex gap-3">
            <button
              ...
              onClick={onClose}
              disabled={isSubmitting}
              className="px-4 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-sm font-bold transition flex-1"
            >
              Batal
            </button>
            <button
